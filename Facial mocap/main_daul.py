...
mainUI = ResPath + "\\resource\FM_mainWindow_daul.ui"
...
class DataThread(QThread):
    ...
    def run(self):
        while not self.stopped:
            if self.subData.live:
                data_from_live = self.subData.sock.recv(64 * 1024)
                self.subData.streamCacheBuffer.setData(data_from_live)
            # if self.subData.rt:
            #     data_from_rt = script.GetFacewareData()
            #     self.subData.streamCacheBuffer2.setData(data_from_rt)
...
class Faceware_live():
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.char_1 = None
        self.char_2 = None
        self.streamCacheBuffer2 = CacheBuffer()
        
    def run(self):
        pass

    def start(self):
        if self.live and self.sock == None:
            global server_ip
            global server_port
    ...
    def loop(self):
        ...
        if self.live:
            received_live = self.streamCacheBuffer.getData()
            if received_live != None:
                received_live = self.extractData(received_live)
                self.analystData(received_live, 0)

        if self.rt:
            received_rt = script.GetFacewareData()
            if received_rt != None or received_rt != "":
                self.analystData(received_rt, 1)

        current_key = script.ConvertTimeToFrame(script.GetCurrentTimeScript())

        if self.live:

            if len(animation_data[0]) != 0:
                script.ProcessFacePuppetKey(self.char_1, current_key,
                                            animation_data[0][0], animation_data[0][1], animation_data[0][2],
                                            animation_data[0][3], animation_data[0][4], animation_data[0][5])

        if self.rt:
            if len(animation_data[1]) != 0:
                script.ProcessFacePuppetKey(self.char_2, current_key,
                                            animation_data[1][0], animation_data[1][1], animation_data[1][2],
                                            animation_data[1][3], animation_data[1][4], animation_data[1][5])
    ...
    def extractData(self, received):
        ...
        try:
            ...
            zeroExpressionData(self.char_1)
            try:
                script.EndFacePuppetKey(self.char_1, script.ConvertTimeToFrame(script.GetCurrentTimeScript()))
            except:
                pass
            scriptEvent.StopTimer()
            return debugMsg("disconnect")
    ...
    def analystData(self, _json, dataType):
        print "analystData"
        ...
        ### Mirror Data ###
        global mirRegularData
        isMirror = fmMainUi.mirror_ckbox.isChecked()
        if isMirror:
            RegularData_m = []
            eyeData_temp = []
            for i in range(60):
                if i > 55 and i < 59:
                    RegularData_m.append(-iCloneRegularData[mirRegularData[i]-1])
                else:
                    RegularData_m.append(iCloneRegularData[mirRegularData[i]-1])
            iCloneRegularData = RegularData_m

            iCloneEyeLData[0] = -iCloneEyeLData[0]
            iCloneEyeRData[0] = -iCloneEyeRData[0]
            iCloneHeadData[1] = -iCloneHeadData[1]
            iCloneHeadData[2] = -iCloneHeadData[2]
            iCloneBoneData[6] = -iCloneBoneData[6]
            iCloneBoneData[9] = -iCloneBoneData[9]
        ...
        global animation_data
        animation_data[dataType] = [iCloneHeadData, iCloneEyeLData, iCloneEyeRData,
                                    iCloneBoneData, iCloneRegularData, iCloneCustomData]
...
def zeroExpressionData(charName):
    tempRegularData = []
    for i in range(60):
    ...
    script.ProcessFacePuppetKey(charName, script.ConvertTimeToFrame(script.GetCurrentTimeScript()),
...
def faceware_connect(device, text):
    if text == "":
        debugMsg("Please Enter Character's Name!")
        return False
...
def startPreview():
    ...
    facewareLive_connected = fmMainUi.checkBox_connect1.isChecked()
    facewareRT_connected = fmMainUi.checkBox_connect2.isChecked()

    if not previewing:
        # Check Device
        if not facewareLive_connected and not facewareRT_connected:
            return debugMsg("Please Check Your Devices!")

        # Check Live Character & Connect
        if facewareLive_connected:
            device.live = True
            try:
                device.start()
            except:
                device.stop()
                return debugMsg("Faceware Live Connect Failed")

            if fmMainUi.lineEdit_picked1.text() != "":
                device.char_1 = fmMainUi.lineEdit_picked1.text()
            else:
                return debugMsg("Please Enter Character's Name!")

        # Check RT Character & Connect
        if facewareRT_connected:
            device.rt = True
            if fmMainUi.lineEdit_picked2.text() != "":
                device.char_2 = fmMainUi.lineEdit_picked2.text()
            else:
                return debugMsg("Please Enter Character's Name!")
        ...
        try:
            if facewareLive_connected:
                script.BeginFacePuppetKey(device.char_1,
                                          script.ConvertTimeToFrame(script.GetCurrentTimeScript()), False)
            if facewareRT_connected:
                script.BeginFacePuppetKey(device.char_2,
                                          script.ConvertTimeToFrame(script.GetCurrentTimeScript()), False)
        except:
            pass
            # debugMsg("BeginFacePuppetKey Error")
        ...
        try:
            if facewareLive_connected:
                zeroExpressionData(device.char_1)
                script.EndFacePuppetKey(device.char_1,
                                        script.ConvertTimeToFrame(script.GetCurrentTimeScript()))
            if facewareRT_connected:
                zeroExpressionData(device.char_2)
                script.EndFacePuppetKey(device.char_2,
                                        script.ConvertTimeToFrame(script.GetCurrentTimeScript()))
        except:
            pass
        scriptEvent.StopTimer()
...
fmMainUi.lineEdit_picked1.setText(script.GetPickedObjectName())
